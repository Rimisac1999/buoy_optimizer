...
def optimize_buoy_allocation(ops_df: pd.DataFrame, buoy_df: pd.DataFrame, 
    ...
    if infeasible_ops:
        print("\n" + "=" * 80)
        print("⚠️  WARNING: OPERATIONS WITH INSUFFICIENT BUOY CAPACITY")
        print("=" * 80)
...
def main():
    ...
    if infeasible_ops:
        ...
        print(f"  ⚠️  Infeasible operations saved to: {infeasible_csv}")
    ...
    if infeasible_ops:
        print(f"\n⚠️  NOTE: {len(infeasible_ops)} operation(s) could not be allocated due to insufficient buoy capacity.")
        ...
        print("\n✅ All operations successfully allocated!")
