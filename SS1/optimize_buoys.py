...
def generate_series_combinations(single_buoys_df: pd.DataFrame, ops_df: pd.DataFrame = None) -> pd.DataFrame:
    ...
    OPTIMIZATION: Only generate series combinations that are actually needed
    based on the submerged weights in the operations data.
    """
    series_configs = []
    
    # Determine the range of upthrust needed
    if ops_df is not None:
        # Be conservative: include any series that could possibly be useful for ANY operation
        max_single_upthrust = single_buoys_df['FinalUpThrust'].max()
        all_sub_w = ops_df[~ops_df['Is_Head']]['Sub W']
        
        # Only generate series for operations that single buoys can't handle alone
        # Add wide margins to be safe
        min_series_needed = max(all_sub_w.min() - DOWN_THRUST_MAX - 5, max_single_upthrust * 0.8)
        max_series_needed = min(all_sub_w.max() - DOWN_THRUST_MIN + 10, 150)
        
        print(f"  Series generation range: {min_series_needed:.1f}t to {max_series_needed:.1f}t upthrust")
    else:
        # Fallback: generate all reasonable combinations
        min_series_needed = 50
        max_series_needed = 150
    
    # Generate all pairs (including same buoy twice)
    for i, buoy1 in single_buoys_df.iterrows():
        for j, buoy2 in single_buoys_df.iterrows():
            ...
            # Skip if outside the useful range
            if combined_upthrust < min_series_needed or combined_upthrust > max_series_needed:
                continue
